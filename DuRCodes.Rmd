...
```{r}
fit.6.m <- lm(Count_m.5 ~ Temp + Humid + WindSpeed + Visibility + DewPointTemp + SolarRad + Rainfall + Snowfall + factor(Season) + factor(Holiday) + factor(FuncDay) + Humid:Visibility + Humid:DewPointTemp + SolarRad:Temp + SolarRad:Humid + Rainfall:Humid, data=bike)
summary(fit.6.m)
extractAIC(fit.6.m)[2]
```
Better R^2 than model withoud interaction, but AIC is too high
...
For AIC(), sqrt still has haighest value. 
